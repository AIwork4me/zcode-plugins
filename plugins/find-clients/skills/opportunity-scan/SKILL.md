...
- **天眼查 `search_bids`** — 招投标与资产处置记录, and the channel that fixes the gap below. Pass `bid_type="4"` for 中标结果 plus `publish_start_time`/`publish_end_time` for the window. Returns 类型 / 标题 / 发布时间 / 中标方 / **项目金额** / 采购人 / 省份 / 公告链接. This is a **record**, so a 中标 found here is `[披露]`, not `[媒体]`.
